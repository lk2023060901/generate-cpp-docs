...
inline std::string get_filename_from_fullpath(const std::string& szFullPath)
{
...
template<class... Args>
static inline std::string attempt_format(const fmt::format_string<Args...>& fmtStr, Args&&... args)
{
    try
    {
    ...
    {
        return fmt::format("format_error:{} fmt:{}", e.what(), fmtStr.get());
    }
...
void printFunctionDetails(CXCursor cursor) {
    ...
    CXString returnTypeName = clang_getTypeSpelling(returnType);
    printf("Return Type: %s\n", clang_getCString(returnTypeName));
    clang_disposeString(returnTypeName);
    ...
    for (int i = 0; i < numArgs; i++) {
        ...
        printf("Parameter %d: %s %s\n", i + 1, clang_getCString(argTypeName), clang_getCString(argName));
...
void printFunctionSignature(CXCursor cursor) {
    CXString functionName = clang_getCursorSpelling(cursor);
    CXType returnType = clang_getCursorResultType(cursor);
    CXString returnTypeName = clang_getTypeSpelling(returnType);

    printf("%s %s(", clang_getCString(returnTypeName), clang_getCString(functionName));
    clang_disposeString(returnTypeName);
    clang_disposeString(functionName);
    ...
    for (int i = 0; i < numArgs; ++i) {
        ...
        printf("%s %s", clang_getCString(argTypeName), clang_getCString(argName));
        clang_disposeString(argTypeName);
        clang_disposeString(argName);
    }
    printf(");\n");
...
// 你需要实现其他部分来遍历 AST 并找到 CXCursor_CXXMethod 类型的 cursor，然后对这些 cursor 调用 printFunctionSignature 函数。


void visit_function(CXCursor cursor, Visitor_Content* pContent)
{
    printFunctionDetails(cursor);
    printFunctionSignature(cursor);
...
// 将 UTF-8 编码的字符串转换为 UTF-16，以便 Windows 控制台可以正确显示
...
void print_utf8_to_console(const std::string& utf8) {
    std::wstring utf16 = utf8_to_utf16(utf8);
    WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), utf16.c_str(), utf16.length(), NULL, NULL);
}
...
void test() {
    try {
        ...
        // 准备 JSON 格式的 POST 数据
        // 请确保 JSON 数据是有效的，这里只是一个示例
        std::u8string u8Data = u8"{\"messages\": [{\"role\": \"user\",\"content\": \"你好\"}]}";
        ...
        print_utf8_to_console(response.str());
    }
...
int main(int argc, char** argv)
{
...

    {
        ...
        else
        {
            ...
            for (auto it = g_export_loc.begin(); it != g_export_loc.end(); ++it) {
                num += it->second.size();
            }
            g_export_loc.clear();
        }
